...
// Rate limiting for profile updates (max 5 per 10 minutes)
$rate_limit = 5;
$rate_limit_time = 600; // 10 minutes
$ip_address = $_SERVER['REMOTE_ADDR'];
if (!isset($_SESSION['profile_update_rate'])) {
    $_SESSION['profile_update_rate'] = [
        'count' => 0,
        'start_time' => time()
    ];
}
if (time() - $_SESSION['profile_update_rate']['start_time'] > $rate_limit_time) {
    $_SESSION['profile_update_rate'] = [
        'count' => 0,
        'start_time' => time()
    ];
...
if (isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error'] === UPLOAD_ERR_OK) {
    if ($_SESSION['profile_update_rate']['count'] >= $rate_limit) {
        $imageMessage = 'Too many profile updates. Please try again later.';
    } else {
        ...
        if (in_array($file_extension, $allowed_extensions)) {
            ...
            if (move_uploaded_file($_FILES['profile_picture']['tmp_name'], $upload_path)) {
                ...
                $_SESSION['profile_update_rate']['count']++;
                log_security_event('PROFILE_PIC_UPDATE', 'Profile picture updated for user: ' . $user_id, $ip_address);
            } else {
    ...
    if ($_SESSION['profile_update_rate']['count'] >= $rate_limit) {
        ...
        if (PasswordUtils::verifyPassword($currentPassword, $storedPassword)) {
            if ($newPassword === $confirmPassword) {
                ...
                $strength = PasswordUtils::validatePasswordStrength($newPassword);
                if (!$strength['valid']) {
                    $message = "Password requirements: " . implode(" ", $strength['errors']);
                } else {
                    $hashed_password = PasswordUtils::hashPassword($newPassword);
                    ...
                    $_SESSION['profile_update_rate']['count']++;
                    log_security_event('PASSWORD_CHANGE', 'Password changed for user: ' . $user_id, $ip_address);
                }
            } else {
